...
function requireResponses(ctx: ExtensionCommandContext): CapturedResponse[] | undefined {
	const responses = getResponsesFromSession(ctx);
	if (responses.length === 0) {
		ctx.ui.notify("No assistant responses found in this session. Ask Pi for a response first.", "warning");
		return undefined;
	}
	return responses;
}
...
async function openPicker(pi: ExtensionAPI, ctx: ExtensionCommandContext, state: ResponseViewerState): Promise<void> {
	const responses = requireResponses(ctx);
	if (!responses) return;
...
export default function responseViewerExtension(pi: ExtensionAPI): void {
	...
	pi.on("turn_end", async (event, ctx) => {
		...
		if (!state.settings.autoOpen) return;
