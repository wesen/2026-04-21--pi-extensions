...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("input", async (event) => {
    // Do not re-process prompts generated by another extension. Current Pi
    // uses "interactive"/"rpc"/"extension" sources, not the old "user" value.
    if (event.source === "extension") {
      log("INPUT", `Skipped: source=${event.source}`);
      return { action: "continue" };
    }
    if (!remindersEnabled) return { action: "continue" };
    if (event.text.includes(USER_PROMPT_REMINDER.trim())) return { action: "continue" };
